...
CollabBoard is a **real-time collaborative whiteboard**. Multiple users share an infinite canvas: they see each other’s cursors, add shapes and sticky notes, move and resize objects, and edit text — all in real time. When you draw a sticky note, as soon as you finish the box it opens for editing with a blinking cursor so you can type immediately; there’s no placeholder text, just the empty note ready for input. We use **Fabric.js** for the canvas and **Supabase** (Postgres + Realtime + Auth) for the backend. Sync is **object-level** (we never send the whole board, only per-object add/change/delete), and we use **dual-layer locking** so two people can’t edit the same object at once — same locking we’ll detail in the Locking section and summarize at the end.

---

## 2. Why Fabric.js? (45 sec)

We use **Fabric.js** for the canvas. The main reason we didn’t use tldraw is **licensing**: tldraw v4+ requires a trial, hobby, or commercial license for production apps. Fabric is **BSD-licensed**, so we can ship without that. The **benefits** of Fabric for us: we get full control over the canvas (pan, zoom, hit-testing, viewport); built-in **serialization** (`toObject` / `enlivenObjects`) that maps cleanly to our JSONB `documents` rows; a clear **event model** (`object:added`, `object:modified`, `object:removed`, plus `object:moving` / `object:scaling` for live drag) so we can hook sync and locking in one place; and we can enable **viewport culling** (`skipOffscreen: true`) for 500+ objects. On the trackpad we use **two-finger scroll for pan** and **pinch for zoom** (zoom at cursor, with tuned sensitivity); mouse wheel still zooms, and we support Hand tool and Space+drag for pan. The trade-off is we own the sync and presence logic ourselves — we kept that manageable with object-level deltas and a single Realtime subscription per board.
