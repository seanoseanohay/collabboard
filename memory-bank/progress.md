...
- **Presence icon avatars in header** — Replace "X others viewing — Alice, Bob" text with pirate emoji icons (same `getPirateIcon(userId)` hash as `CursorOverlay`). Show up to 4 icons, then "+N" overflow badge. Hover any icon → tooltip with name. Click icon → pan/zoom canvas to that user's cursor position. Count text ("X others viewing") shows only on hover of the cluster. Files: `WorkspacePage.tsx` header presence area; export/share `getPirateIcon` util.
